...
from search import download_webpages_ddg, parse_html_to_text, group_web_results_by_domain, sanitize_filename
...
monte_carlo_search = CONFIG["monte_carlo_search"]
...
def call_gemma(prompt, temperature=0.3, personality=None,chat_history =None):   
    """
    Calls the LLM with optional chat history and temperature.
    """
    system_message = ""
    if personality:
    ...
    tokens_used = token_counter(model=AZURE_MODEL_DEPLOYMENT, messages=messages)
    
    response = completion(
        model =f"azure/{AZURE_MODEL_DEPLOYMENT}",
        messages=messages, 
...
class SearchSession:
    def __init__(self, query, top_k=3, web_search_enabled=False, personality=None, rag_model="gemma", max_depth=1):
        """
        ...
        self.base_result_dir = os.path.join("tmp/results", self.query_id)
        ...
        self.enhanced_query_embedding = embed_text(self.enhanced_query)
    ...
    async def run_session(self):
        ...
        initial_subqueries = self.generate_subqueries(plain_enhanced_query)
        initial_subqueries = self.maybe_monte_carlo(initial_subqueries, plain_enhanced_query)

        if self.web_search_enabled and self.max_depth >= 1:
            await self.run_web_search(initial_subqueries)
        else:
    ...
    def generate_final_answer(self):
        """
        Generate the final answer based on web and local results.
        Optionally include previous results and follow-up conversation.
        """
        summarized_web = self._summarize_web_results(self.web_results)
        summarized_local = self._summarize_local_results(self.local_results)
        final_answer = self._build_final_answer(summarized_web, summarized_local)
    ...
    def generate_subqueries(self, plain_enhanced_query):
        # 1) Generate subqueries from the enhanced query
        initial_subqueries = split_query(plain_enhanced_query, max_len= 200)
        print(f"[INFO] Generated {len(initial_subqueries)} initial subqueries from the enhanced query.")
        return initial_subqueries

    def maybe_monte_carlo(self, initial_subqueries, plain_enhanced_query):
        ...
        if monte_carlo_search:
            ...
            return self.perform_monte_carlo_subqueries(plain_enhanced_query, initial_subqueries)
        return initial_subqueries

    async def run_web_search(self, initial_subqueries):
        ...
        for qu in web_entries:
            self.kb.add_documents(qu["text"],query_id=self.query_id)
    ...
    async def perform_monte_carlo_subqueries(self, parent_query, subqueries):
        """
    ...
    async def perform_recursive_web_searches(self, subqueries, current_depth=1):
        ...
        for sq in subqueries:
            ...
            # Relevance
            node_embedding = embed_text(sq_clean)
            ...
            pages = await download_webpages_ddg(sq_clean, limit=web_search_limit, output_dir=subquery_dir)
            branch_web_results = []
            ...
            if current_depth < self.max_depth:
                ...
                if additional_query and additional_query != sq_clean:
                    additional_subqueries = split_query(additional_query, max_len=max_query_length)
